...
def create_orders
  ...
  @my_shopify_orders = ShopifyOrder.where('created_at BETWEEN ? AND ? AND sent_to_acs_at = ?', 10.days.ago.utc, Time.now.utc, nil)
  ...
  # for each ShopifyOrder in DB, create at least one 'order' -- if ShopifyOrder only has one line item, only one order will be created, while multiple 'orders' (CSV rows) will be created if ShopifyOrder contains multiple line items
...
def fill_csv(orders)
  ...
  CSV.open(filename,"w+") do |file|
    ...
    orders.each do |order|
      ...
      # skipped indices are blank (legacy) in output CSV
...
# IMPORTANT INDEX REFERENCE FOR CSV:
# 0 order number
# 2 order date
# 3 merchant sku
